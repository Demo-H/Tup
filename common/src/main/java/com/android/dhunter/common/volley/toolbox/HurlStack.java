...
import com.android.dhunter.common.volley.Request.Method;
    ...
    public final org.apache.http.HttpResponse performRequest(Request<?> request, Map<String, String> additionalHeaders)
        ...
        for (Map.Entry<String, List<String>> header : connection.getHeaderFields().entrySet()) {
            if (header.getKey() != null) {
                org.apache.http.Header h = new BasicHeader(header.getKey(), header.getValue().get(0));
                response.addHeader(h);
                if (header.getKey().equalsIgnoreCase("Set-Cookie")){
                    String cookie="";
                    for (int i = 0; i < header.getValue().size(); i++) {
//                        cookie += header.getValue().get(i)+";";
                        CookieManager.getInstance().setCookie(url, header.getValue().get(i));
