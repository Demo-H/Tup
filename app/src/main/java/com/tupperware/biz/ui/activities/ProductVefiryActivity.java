...
import com.tupperware.biz.ui.component.DaggerProductVefiryActivityComponent;
...
import com.tupperware.biz.ui.presenter.ProductVefiryPresenter;
...
//    TextView productVerifyTextView;
    ...
    private int isUpgrade = 0; // 1：是，0：否，默认0 ,
...
//        String json = (String) mSharePreDate.getParam(Constants.KEY_DATA_SCAN_COUPON, "");
        ...
        if (verifyCoupon != null && verifyCoupon.model != null) {
            if (TextUtils.isEmpty(verifyCoupon.model.getBenefitFrontName())) {
                msgTextview.setText("暂无");
            } else {
                msgTextview.setText(verifyCoupon.model.getBenefitFrontName());
            }
            explainLinLayout.setVisibility(View.VISIBLE);
            String url = ImageUrl.parseUrl(verifyCoupon.model.getImageurl());
            if(url != null || !url.isEmpty()) {
                couponImageView.setImageURI(url);
                mDaijinquanText.setVisibility(View.GONE);
            } else {
                mDaijinquanText.setVisibility(View.VISIBLE);
            }
        }
    }
...
//            int width = getResources().getDimensionPixelSize(R.dimen.dimen_120dp);
            String url = verifyProduct.getModel().getProductInfo().getProductImage();
            if(!url.isEmpty()) {
                url = ImageUrl.parseUrl(url);
                productImageView.setImageURI(url);
            }
            productCodeTextView.setText(productCode);
            productNameTextView.setText(verifyProduct.getModel().getProductInfo().getProductNameCn());
            productPriceTextView.setText("￥ " + String.format("%.2f", verifyProduct.getModel().getProductInfo().getProductContentPrice()));
...
//            BaseResponse mHeader = BaseResponse.createInstanceByJson(json);
//            if (mHeader == null) {
//                toast(getString(R.string.system_error));
//                return;
//            }
        ...
        switch (view.getId()) {
            ...
            case R.id.activity_productvefiry_affirm_btn:
                ...
                if(isCanUpdate()) {
                    ...
                    //联网确认使用
                    mPresenter.useProductCoupon(Constants.SCAN_COUPON_ONLY_CODE, productCode, memberId, isUpgrade);
                    showDialog();
                }
                //粉丝注册
                /*if(Constant.DemoF) {
...
//                        asyncLoading(Constants.REQUEST_CODE_USE_COUPON, productCode);
                        mPresenter.useProductCoupon(Constants.SCAN_COUPON_ONLY_CODE, productCode, getMemberId());
                        showDialog();
                    }
                }*/
                break;
...
//            String json = (String) mSharePreDate.getParam(Constants.KEY_DATA_SCAN_PRODUCT_JSON, "");
        ...
        if(verifyProduct != null) {
            if(verifyProduct.getModel().getMembers().getGroupId() == 2) {
                return true;
            } else {
                return false;
            }
    ...
    private Integer getMemberId() {
        ...
        if(verifyProduct != null) {
            memberId = verifyProduct.getModel().getMembers().getMemberId();
        }
        return memberId;
    }
    ...
    private boolean isCanUpdate() {
        ...
        if (verifyProduct != null && verifyProduct.getModel() != null) {
            memberId = verifyProduct.getModel().getMembers().getMemberId();
            mDataManager.saveSPObjectData(Constants.FANS_MEMBER_ID, memberId);
            if(verifyProduct.getModel().getProductInfo() != null && verifyProduct.getModel().getMembers() != null) {
                if ("VIP".equals(verifyProduct.getModel().getProductInfo().getMemberGroupId().toUpperCase()) &&
                        verifyProduct.getModel().getMembers().getGroupId() != 1) {
                    //产品为高会产品，会员为非高级会员
                    return true;
                }
            }
