...
import com.tupperware.biz.widget.UpdateDialog;
...
public class UpdateUtils {
    ...
    public static void startDownload(@NonNull String url, final Context context) {
        ...
        initBuilder(context);
        final String dirPath = FileUtils.DIR_DOWNLOAD;
        if (dirPath.length() > 0) {
            int index = url.lastIndexOf("/") + 1;
            if (index > -1 && index < url.length() - 1) {
//                final String fileName = url.substring(index);
                final String fileName = FileUtils.APP_NAME + ".apk";
...
//                        return;
                    ...
                    FileDownloader.getImpl().create(url).setTag(fileName).setPath(filePath).setListener(new FileDownloadListener() {
                        ...
                        protected void progress(BaseDownloadTask task, final int soFarBytes, final int totalBytes) {
                            ...
                            builder.setContentText(String.format(context.getString(R.string.download_progress), (int)progress));
...
//                .setTicker(context.getString(R.string.version_update));
    }
